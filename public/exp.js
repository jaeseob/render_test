...
function make_report_obj(radi = 4, PPC = 35, color = 'rgb(80, 80, 80)'){
    ...
    let lineWidth = Math.round(radi * PPC * 0.1)
    let circleRad = Math.round(radi * PPC * 0.15)
...
class MG{
    ...
    make_blob(){
        ...
        let gaussSig = this.gaussSig * PPC / 3*2
        let baseBright = 128
...
let S, d, gbs, report_obj, reportDiv, noise1, noise2
...
async function setup_experiment(){
    ...
    let dw = Math.round(PPC * 10)
    let dh = Math.round(PPC * 15)
    ...
    fixation.style.left = '20%'
    fixation.style.top = '30%'
...
let n1, n2
async function run_oneTrial(targ, targLoc = -1, centerLoc = -1, startTime = Date.now()){
    let oriCand = cfg.oris
    let nontarg = oriCand[Math.floor(Math.random()*oriCand.length)]

    gbs[0].style.transform = `translate(-50%, -50%) rotate(${targ}deg)`
    gbs[1].style.transform = `translate(-50%, -50%) rotate(${nontarg}deg)`
    let cx = parseInt(d.style.width)/2
    let cy = parseInt(d.style.height)/2
    
    // middle.style.left = `${cx}px`
    // middle.style.top = `${cy}px`
    // middle.style.left = '50%'
    // middle.style.top = '50%'
    // gbs[0].style.top = `${cy + targLoc*locDiff}px`
    // gbs[1].style.top = `${cy - targLoc*locDiff}px`
    ...
    // gbs[0].style.left = `${cx + targLoc*locDiff}px`
    // gbs[1].style.left = `${cx - targLoc*locDiff}px`

    gbs[0].style.top = `${cy + targLoc*locDiff}px`
    gbs[1].style.top = `${cy - targLoc*locDiff}px`

    
    // d.style.left = `${gcx + cfg.xCen * PPC + centerLoc*cenDiff}px`
    // d.style.top = `${gcy - cfg.yCen * PPC + centerLoc*cenDiffV}px`
    ...
    // d.style.left = `${gcx + 10*PPC}px`
    // d.style.top = `${gcy  + centerLoc*cenDiff}px`
...
async function run_multiTrial(){
    ...
    for(let iBlock = 0; iBlock < cfg.nBlock; iBlock++){
        ...
        for(let iTrial = 0; iTrial < cfg.nTrial; iTrial ++){
            ...
            centerLoc = centerLoc * -1 // update center Loc

            targOri = cfg.oris[Math.floor(Math.random() * cfg.oris.length)]

            trial_result = await run_oneTrial(targOri, targLoc, centerLoc, Date.now()+500)
